...
The starter files generated by `playdsp new` include commented-out working examples of both patterns.

### Rust — `LazyLock<Mutex<T>>`

Declare a state struct and wrap it in a `LazyLock<Mutex<...>>` static. The static is initialised exactly once (on the first buffer call) and lives for the entire run:
...
Size per-channel `Vec`s lazily — the channel count is only known at call time, not at static initialisation.

### C++ — `static` local struct

Declare a struct and mark its instance `static` inside `cpp_process()`. C++ guarantees it is constructed exactly once:
